...
char* pms(int *size, Pizza *pizzas) {
    ...
    snprintf(result, 128, "Best Sold Pizza: %s (Total Quantity: %d)", best_sold_pizza, max_quantity);
    return result;
}
//Pls Worst sold pizza as string
char* pls(int *size, Pizza *pizzas) {
    ...
    char *result = malloc(128);
    snprintf(result, 128, "Worst Sold Pizza: %s (Total Quantity: %d)", worst_sold_pizza, min_quantity);
    return result;
}
//Apo Average pizzas per order
